...
On a local-markdown tracker, comments are written into a committed `.md` file, so hard-wrap every
comment and triage note to ≤100 characters per line (prose only — leave tables and fenced code
blocks intact) to satisfy markdownlint MD013. See `issue-tracker-local.md`.
