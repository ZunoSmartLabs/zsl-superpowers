...
7. **Synthesize the timesheet.** Write the outcome bullets and the "How the day went" narrative per the rules below. With Harvest connected they are the raw material for the rows' notes and are **not printed on their own**; without Harvest they are the deliverable, printed with the standard header.

8. **Propose Harvest entries.** If the Harvest MCP tools exist (`mcp__harvest__list_projects`, `mcp__harvest__list_project_assignments`, `mcp__harvest__list_time_entries`, `mcp__harvest__log_time`), build the entries per the Harvest rules below, check `list_time_entries` for the window's dates so nothing already logged is proposed twice, and print: one sentence saying what the table covers (the date, the sources read, whether anything is already logged), the entries table, the summary table, the unaccounted line, any questions, and *"Log these N entries to Harvest?"*. No timesheet render, no narrative, no preamble beyond that sentence. **Never call `log_time` before the user has said yes to the table**; on yes, log each row with `spent_at`, `started_time`/`ended_time`, `project_id`, `task_id` and `notes`, then confirm the ids. No Harvest tools → end with *"Copy this to your clipboard?"* and on yes `printf '%s' "<bullets>" | pbcopy` (macOS) / `wl-copy` / `xclip -selection clipboard` / `clip` (Windows).
...
- **Same customer, same minutes, one row.** When a meeting and a block belong to the same customer and overlap in time, they merge into one row spanning both, with both sets of notes; a huddle held while working in the repo is one piece of time, not two. Overlaps across different customers stay separate rows and are flagged.
