...
- **Reconciles the ledger; doesn't run workers.** `/afk-fanout` writes the manifest + initial entries, `/afk-worker` writes per-PRD outcomes — both on `afk-runs`. This skill reads that ledger, **replays** it into canonical `.scratch/` on `main`, and writes back only the `reconciled:` marker. All three must agree on the `afk-runs` `.afk-runs/<date>/` layout and the entry schema (see `/afk-fanout`'s SKILL.md).
