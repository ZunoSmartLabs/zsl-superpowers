...
|---|---|---|---|
| **Pre-flight refusal** | Step 1d found an in-scope story with no `AC<n>:` acceptance criterion, or **no slice is runnable** (every slice deferred behind an open `[HITL]` or still in triage) | PRD pre-dates the acceptance-criteria requirement, or every slice is gated and `/zsl:human-itl` hasn't been run yet. (Open `[HITL]` slices *alongside* runnable ones no longer refuse — they trigger a partial run.) | No branch state to inspect — refused before any work. Fix the PRD or run `/zsl:human-itl <parent>` and re-invoke. |
