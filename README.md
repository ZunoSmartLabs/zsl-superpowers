...
- **[`/verify-coverage`](./skills/engineering/verify-coverage/SKILL.md) `<PRD>`** — after the fanout integrates the slices, check every PRD `## User Stories` entry against the *implemented code via tests*, not prose. Maps each story to a passing behavioral test (Tier A); for stories with none, generates a test, proves it non-vacuous by mutation, and runs it (Tier B); routes visual/UX/external stories to a human-attestation lane (HITL); quarantines the failing tests and auto-files the genuine gaps as `needs-triage` sub-issues of the PRD, and writes a coverage receipt against the verified sha. **`/tdd-parallel` enforces this**: step 4b refuses to open the integration PR until a valid receipt for the integrated tip exists. It's an *execution* gate (did the check run?), not an *outcome* gate — open gaps still pass; only skipping the check is blocked. The matrix outcome itself remains a review surface, never an auto-gate.
...
- **[human-itl](./skills/engineering/human-itl/SKILL.md)** — Walk a human through the manual-action `[HITL]` slices `/tdd-parallel` skipped (console clicks, credential rotation, sign-off), record each as an audit trail, mark them done so the dependent `[AFK]` slices unblock, then hand back to the fanout. Hard-refuses disguised-decision slices.
- **[to-issues](./skills/engineering/to-issues/SKILL.md)** — Break any plan, spec, or PRD into independently-grabbable GitHub issues using vertical slices.
- **[to-prd](./skills/engineering/to-prd/SKILL.md)** — Turn the current conversation context into a PRD and submit it as a GitHub issue. No interview — just synthesizes what you've already discussed.
- **[verify-coverage](./skills/engineering/verify-coverage/SKILL.md)** — After a fanout, verify every PRD user story is covered by a passing, non-vacuous behavioral test (generating and proving one where none exists), route non-automatable stories to a human-attestation lane, and auto-file genuine gaps as triage issues.
