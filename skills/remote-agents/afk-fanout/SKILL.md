...
- **`job_config.ccr.session_context.sources` selects the repo this routine clones** — and the nesting must be **inside `session_context`**, not at the `ccr` level. Confirmed three ways by live API (2026-05-31): (a) an existing scheduled routine read via `RemoteTrigger({action:"list"})` carries `session_context.sources: [{ git_repository: { url } }]` with the repo URL; (b) a dedicated disabled, far-future probe (`trig_01XNjbsrAaFZRdn9tdt4ctTM`) created with `sources` inside `session_context` echoes the URL back verbatim on `get`; (c) a `create` that instead placed `sources` at the `ccr` level (a sibling of `session_context`) was **rejected HTTP 400** — `translate job_config v1→v2: … unknown field "sources"`. So it's a hard error, not a silent drop: get the nesting exactly right. Without `sources` the routine fires with no checkout and `/afk-worker` → `/tdd-parallel` has nothing to operate on — the bug this skill exists to avoid. The URL is the pre-flight-resolved `<origin https URL>`; never hardcode it (`/afk-fanout` runs against whatever repo the PRDs live in).
